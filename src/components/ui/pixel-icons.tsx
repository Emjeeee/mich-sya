...
// Maps a bitmap char to a fixed color. Chars with no entry (or icons with no
// palette at all) fall back to `currentColor`, so purely functional glyphs
// (chevrons, send, check...) keep following text-color/hover/active state —
// only the representational icons (nav, dashboard, arcade) get a fixed,
// "colorful" identity, since hardcoding those would break state-driven
// coloring and can make an icon disappear against a same-colored button.
...
function PixelIcon({
  ...
  palette,
  ...props
}: { bitmap: Bitmap; palette?: Palette } & SVGProps<SVGSVGElement>) {
  const size = bitmap.length
  const cells: { x: number; y: number; fill: string }[] = []
  bitmap.forEach((row, y) => {
    row.split('').forEach((c, x) => {
      if (c !== '.') cells.push({ x, y, fill: palette?.[c] ?? 'currentColor' })
    })
  })
  return (
    <svg viewBox={`0 0 ${size} ${size}`} shapeRendering="crispEdges" {...props}>
      {cells.map((c) => (
...
const ICONS = {
  ...
  // Yellow/red discs, like the real game — cheap to do since the dots
  // already sit on isolated cells, just alternate the fill char per dot.
  connect4: def(
  ...
  // Simon Says' colorful pad — two blobs, two colors.
  pad4: def(
